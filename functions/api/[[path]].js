...
export async function onRequest({ request, env }) {
  try {
    ...
    if (path === 'weekly-plan') return json(await h.weeklyPlan(ctx, env, url, body, method), method === 'POST' ? 201 : 200);
    if (path === 'my-service') return json(await h.myService(ctx, env, url, body, method), method === 'POST' ? 201 : 200);
