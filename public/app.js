...
async function myService(){
  ...
  const schedule=(d.schedule||[]).map(x=>{const dt=new Date(x.service_date+'T00:00:00');return `<div class="schedule-row"><div class="schedule-date"><b>${dt.toLocaleDateString('en-ZA',{weekday:'short'})}</b><span>${dt.getDate()}</span></div><div><b>${esc(x.facility_name)}</b><span>${esc(x.service_focus||'')}</span></div></div>`}).join('');
