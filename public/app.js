...
function referralModal(item, internsData) {
  ...
  modal(isEdit?'Update referral':'Add referral',`<form id="fReferral" class="formgrid">${isEdit?`<input type="hidden" name="id" value="${item.id}">`:''}
    ...
    ${!isEdit?`<div class="field"><label>De-identified referral code<input name="referral_code" placeholder="e.g. EG-024" maxlength="50" required></label></div><div class="field"><label>Date allocated<input name="referral_date" type="date" value="${today()}" required></label></div><div class="field"><label>Referral source<select name="referral_source"><option>CAReS</option><option>Tuesday allocation</option><option>Social worker</option><option>Inpatient team</option><option>Clinic</option><option>Other</option></select></label></div><div class="field"><label>Site<input name="site" maxlength="120"></label></div><div class="field"><label>Presenting category<input name="presenting_category" maxlength="120" placeholder="Broad category only"></label></div>`:''}
  ...
  document.addEventListener('submit',saveReferral,{once:true});
}
async function saveReferral(e){if(e.target.id!=='fReferral')return;e.preventDefault();const item=Object.fromEntries(new FormData(e.target));try{await api('referrals',{method:item.id?'PATCH':'POST',body:JSON.stringify(item)});closeModal();toast(item.id?'Referral updated':'Referral added');go('referrals');}catch(x){toast(x.message);document.addEventListener('submit',saveReferral,{once:true});}}
...
function openingBalanceModal(component, id) {
  ...
  document.addEventListener('submit', saveOpeningBalance, { once: true });
}
...
async function cases() {
  ...
  $('#addCase')?.addEventListener('click', () => { modal('Allocate de-identified case', `<form id="fCase" class="formgrid"><input type="hidden" name="intern_profile_id" value="${id}"><div class="field"><label>Case code<input name="case_code" placeholder="KHC-026" required></label></div><div class="field"><label>Site<input name="site" required></label></div><div class="field"><label>Presenting category<input name="presenting_category"></label></div><div class="field"><label>Planned frequency<select name="planned_frequency_weeks"><option value="1">Weekly</option><option value="2">Fortnightly</option><option value="4">Monthly</option></select></label></div><div class="full"><button class="btn primary">Allocate</button></div></form>`); document.addEventListener('submit', saveCase, { once: true }); });
}
async function saveCase(e) { if (e.target.id !== 'fCase') return; e.preventDefault(); await api('cases', { method: 'POST', body: JSON.stringify(Object.fromEntries(new FormData(e.target))) }); closeModal(); toast('Case allocated'); go('cases'); }
function activityModal(c) { modal('Record counselling activity · ' + c.case_code, `<form id="fActivity" class="formgrid"><input type="hidden" name="intern_profile_id" value="${c.intern_profile_id}"><input type="hidden" name="case_id" value="${c.id}"><div class="field"><label>Date<input name="encounter_date" type="date" value="${today()}" required></label></div><div class="field"><label>Session<select name="session_type"><option>First</option><option>Follow-up</option></select></label></div><div class="field"><label>Booked<select name="booked"><option value="true">Yes</option><option value="false">No</option></select></label></div><div class="field"><label>Attended<select name="attended"><option value="true">Yes</option><option value="false">No</option></select></label></div><div class="field"><label>Duration if attended (minutes)<input name="duration_minutes" type="number" min="1" max="480" value="60"></label></div><div class="field"><label>Gender for monthly statistics<select name="patient_gender"><option>Female</option><option>Male</option><option>Other</option><option>Unknown</option></select></label></div><div class="full"><button class="btn primary">Save activity</button></div></form>`); document.addEventListener('submit', saveActivity, { once: true }); }
async function saveActivity(e) { if (e.target.id !== 'fActivity') return; e.preventDefault(); await api('encounters', { method: 'POST', body: JSON.stringify(Object.fromEntries(new FormData(e.target))) }); closeModal(); toast('Session recorded'); go('cases'); }
...
async function feedbackView(){
  ...
  $('#fFeedback')?.addEventListener('submit',async e=>{e.preventDefault();const b=Object.fromEntries(new FormData(e.target));b.context_view=S.view;await api('feedback',{method:'POST',body:JSON.stringify(b)});toast('Feedback saved');go('feedback');});
}
...
function demoProgramme(d){return{metrics:{interns:1,cases:0,active_cases:0,hours:470.5,at_risk_interns:0,open_supervision:0,reviewed_reports:0,booked:0,attended:0,attendance_rate:null,median_days_to_intake:null},sites:[],institutions:[{institution:'SACAP',count:1}]};}
